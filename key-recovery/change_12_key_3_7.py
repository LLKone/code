...
m6 = np.load(wdir + "data_wrong_key_mean_6r_pairs_num100000.npy");
s6 = np.load(wdir + "data_wrong_key_std_6r_pairs_num100000.npy"); s6 = 1.0/s6;
...
def make_structure(pt0l, pt0r, pt2l, pt2r,diff=(0x8060,0x4101),neutral_bits = [[20],[22],[13],[12,19],[14,21],[6,29]]):
...
def gen_challenge(n, nr, keys_list, diff=(0x8060,0x4101), neutral_bits = [[20],[22],[13],[12,19],[14,21],[6,29]], keyschedule='real'):
  # 随机生成明文对(pt0,pt1)，数据类型为无符号16位整型
...
def find_good(cts, key, nr=4, target_diff = (0x0040,0x0)):
  # 解密运算
...
def run_bayes(cts,it=1, cutoff1=7, cutoff2=10, net=net7, net_help=net6, m_main=m7, m_help=m6, s_main=s7, s_help=s6, verify_breadth=None):
  n = len(cts[0]);
...
def run(n, nr=12, num_structures=10000000, it=500, cutoff1=10.0, cutoff2=10.0, neutral_bits=[[20],[22],[13],[12,19],[14,21],[6,29]], keyschedule='real',net=net7, net_help=net6, m_main=m7, s_main=s7,  m_help=m6, s_help=s6, verify_breadth=None):
