...
def train_speck_distinguisher(num_epochs, num_rounds=7, depth=1, folder='./'):
    net = load_model("7_distinguisher_Inceptions_nets_change_depth_2.h5")
    ...
    net_first.load_weights("7_distinguisher_Inceptions_nets_change_depth_2.h5")

	Numm1 = 0;
	while(True):
        X1, Y1, C1 = sp.make_train_data(10 ** 8, num_rounds-3, diff=(0x8000, 0x840a))  
        X2, Y2, C2 = sp.make_train_data(10 ** 8, num_rounds-3, diff=(0x8000, 0x840a))		
		if(Numm1 < 10 ** 7):
			Numm1 += C1 + C2;
	...
	Numm2 = 0;
	while(True):
        X_eval1, Y_eval1, C_eval1 = sp.make_train_data(10 ** 8, num_rounds-3, diff=(0x8000, 0x840a)) 
        X_eval2, Y_eval2, C_eval2 = sp.make_train_data(10 ** 8, num_rounds-3, diff=(0x8000, 0x840a))	
		if(Numm2 < 10 ** 7):
    ...
    # net.save(folder + str(num_rounds) + '_distinguisher_change_nets_depth_' + str(depth) + '.h5')
...
def second_stage(num_rounds=8):
	...
	Numm3 = 0;
	while(True):
        X1, Y1, C1 = sp.make_train_data(10 ** 8, num_rounds)  
        X2, Y2, C2 = sp.make_train_data(10 ** 8, num_rounds)		
		if(Numm3 < 10 ** 7):
			Numm3 += C1 + C2;
			X = np.concatenate((X1, X2), axis=0)
			Y = np.concatenate((Y1, Y2), axis=0)
	...
	Numm4 = 0;
	while(True):
        X_eval1, Y_eval1, C_eval1 = sp.make_train_data(10 ** 8, num_rounds)  
        X_eval2, Y_eval2, C_eval2 = sp.make_train_data(10 ** 8, num_rounds)		
		if(Numm4 < 10 ** 7):
...
if __name__ == "__main__":
    ...
    train_speck_distinguisher(50, num_rounds=8)
