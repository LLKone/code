...
def train_speck_distinguisher(num_epochs, num_rounds=7, depth=1, folder='./'):
	...
	Numm1 = 0;
	while(True):
        X1, Y1, C1 = sp.make_train_data(10 ** 7, num_rounds)  
        X2, Y2, C2 = sp.make_train_data(10 ** 7, num_rounds)		
		if(Numm1 < 10 ** 7):
			Numm1 += C1 + C2;
	...
	Numm2 = 0;
	while(True):
        X_eval1, Y_eval1, C_eval1 = sp.make_train_data(10 ** 7, num_rounds)  
        X_eval2, Y_eval2, C_eval2 = sp.make_train_data(10 ** 7, num_rounds)		
		if(Numm2 < 10 ** 7):
    ...
    # early_stopping = EarlyStopping(monitor='val_loss', patience=50)  # Early stopping
    ...
    net.save(folder + str(num_rounds) + '_distinguisher_Inceptions_nets_change_depth_' + str(depth) + '.h5')
